...
public class SecurityConfig {
  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http
      ...
      .authorizeRequests()
      .requestMatchers("/orders/ship").hasAuthority(EshopRole.Admin)
      .requestMatchers("/orders/cancel").hasRole(EshopRole.Admin)
      .requestMatchers(HttpMethod.GET, "/orders").hasRole(EshopRole.Admin)
      .requestMatchers(HttpMethod.GET, "/orders/*").hasAuthority(scope(ORDERS_SCOPE))
      .requestMatchers(HttpMethod.POST, "/orders/*").hasAuthority(scope(ORDERS_SCOPE))
      .requestMatchers(HttpMethod.PUT, "/orders/*").hasAuthority(scope(ORDERS_SCOPE))
      .requestMatchers(HttpMethod.DELETE, "/orders/*").hasAuthority(scope(ORDERS_SCOPE))
      .and()
